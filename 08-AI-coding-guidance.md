...
```text
"There is an error in the loss function implementation in compute_loss. Fix the equation so it correctly implements mean square error loss."
```
...
```text
"I need to implement a change to the normalization functions in this repository. Read the relevant files and explain how they work before suggesting changes."
```
